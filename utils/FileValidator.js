class FileValidator {
  ...
  static max = (file, fieldName, maxSize) => {
    if (file.size > maxSize) {
      throw new Error(`The ${fieldName} file max size is ${maxSize/1000000} MB!`);
    }
  }

  static min = (file, fieldName, minSize) => {
    if (file.size < minSize) {
      throw new Error(`The ${fieldName} file min size is ${minSize/1000000} MB!`);
    }
  }

  static mimetypes = (file, fieldName, validMimetypes) => {
    if (!validMimetypes.includes(file.mimetype)) {
      throw new Error(`The ${fieldName} file mimetype should be ${validMimetypes.join(' or ')}!`);
    }
  }

  static maxFiles = (files, fieldName, maxFileCount) => {
    if (files.length > maxFileCount) {
      throw new Error(`The ${fieldName} field max file count should be ${maxFileCount}`);
    }
  }
